...
| **Prayer Points** | Two things shown per day, each on its own one-per-day rotation: a Title + Content topic from the `Prayer_Points` sheet, plus a name to pray for from the `Prayer_People` sheet — two columns, **Church** and **Outside Church**, purely for your own bookkeeping; the app concatenates both into one sequence and just rotates through the names in order, without ever showing which column a name came from. |
